...
async function getCompData(url, page) {
  await page.goto(url);
  const name = await page.$eval(".table tbody td:nth-child(2) p", p => p.textContent);
  const status = await page.$eval(".table tbody tr:nth-child(2) td:nth-child(2) p", p => p.textContent);
  const cat = await page.$eval(".table tbody tr:nth-child(5) td:nth-child(2) p", p => p.textContent);
  const coc = await page.$eval(".table tbody tr:nth-child(7) td:nth-child(2) p", p => p.textContent);
  const doi = await page.$eval(".table tbody tr:nth-child(8) td:nth-child(2) p", p => p.textContent);
  const director = await page.$eval("tr#package1 td:nth-child(2) p a", a => a.textContent);
  ...
  for (const table of tables) {
    const rows = await table.$$("tr");
    for (const row of rows) {
      const cells = await row.$$eval("td p", tds => tds.map(td => td.textContent.trim()));
      if (cells.length === 2) {
        const [key, value] = cells;
        if (key === "Authorised Capital") {
          authorisedCapital = value;
  ...
  if (cfEmailElement) {
    const cfEmailEncodedString = await page.evaluate(element => element.getAttribute("data-cfemail"), cfEmailElement);
...
app.post("/scrape", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: `An error occurred during scraping: ${error.message}` });
  } finally {
    if (browser) await browser.close();
  }
});
